...
def main():
    ...
    with torch.no_grad():
        ...
        for stdy_idx, data in enumerate(test_loader):
            ...
            flow_scaled = flow*2
            fbct_def = reg_model([fbct, flow_scaled]) 
            # flow = flow / flow.abs().max()   # Normalizacija na interval [-1, 1]
            print(f"Flow stats: mean={flow_scaled.mean().item()}, max={flow_scaled.max().item()}, min={flow_scaled.min().item()}")

    
            tar = fbct.detach().cpu().numpy()[0, 0, :, :, :]
            
            jac_det = utils.jacobian_determinant_vxm(flow_scaled.detach().cpu().numpy()[0, :, :, :, :])
            print(f"Flow stats for patient {stdy_idx + 1}: mean={flow_scaled.mean().item()}, max={flow_scaled.max().item()}, min={flow_scaled.min().item()}")
            ...
            visualize_registration(
                fixed=cbct[0, 0],
                moving_image=fbct[0, 0],
                moving_transformed=fbct_def[0, 0],
                deformation_field=flow_scaled[0],
                stdy_idx=stdy_idx
            )
            
            flow_numpy = flow_scaled[0].detach().cpu().numpy()  # Oblika: [3, D, H, W]
