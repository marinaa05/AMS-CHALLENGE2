...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow):
    # Interpolate flow to match the grid size if dimensions do not match
        if flow.shape[2:] != self.grid.shape[2:]:
            flow = nn.functional.interpolate(flow, size=self.grid.shape[2:], mode='trilinear', align_corners=True)
